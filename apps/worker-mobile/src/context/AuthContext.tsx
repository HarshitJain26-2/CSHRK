  ...
  const login = async (email: string, pass: string): Promise<boolean> => {
    ...
    try {
      ...
      if (session.user.role !== UserRole.WORKER) {
        setError(`Access restricted: Worker application requires WORKER role. Current role: ${session.user.role}`);
        return false;
      }
      ...
      const message =
        err.response?.data?.message || 'Login failed. Please verify worker credentials.';
      setError(message);
      return false;
    } finally {
      setIsLoading(false);
